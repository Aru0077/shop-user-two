<template>
    <div class="w-full max-w-md mx-auto">
      ...
      <Disclosure v-for="category in categories" :key="category.id" as="div" 
        class="mt-3 overflow-hidden shadow-sm transition-all duration-200 hover:shadow-md" 
        v-slot="{ open }"
      >
        <DisclosureButton
        ...
        >
          ...
          <ChevronUpIcon 
            :class="open ? 'rotate-180 transform text-black-600' : 'text-gray-400'" 
            class="h-5 w-5 transition-transform duration-300 ease-in-out" 
          />
        </DisclosureButton>
        <DisclosurePanel class="border-l-4 border-black-100 bg-white px-5 pt-3 pb-4 text-sm text-gray-600 divide-y divide-gray-100">
          <!-- 显示子分类 -->
          <div 
      ...
      <div v-if="loading" class="mt-6 text-center text-gray-500 py-8 bg-white rounded-md shadow-sm">
        <div class="inline-block h-5 w-5 animate-spin rounded-full border-2 border-solid border-indigo-500 border-r-transparent mr-2 align-middle"></div>
        <span>加载分类中...</span>
      </div>
